...
import { validatedIpcMain } from 'vs/base/parts/ipc/electron-main/ipcMain.patch';
...
export class Server extends IPCServer {
	...
	private static getOnDidClientConnect(): Event<ClientConnectionEvent> {
		const onHello = Event.fromNodeEventEmitter<WebContents>(validatedIpcMain, `${projectName}:hello`, ({ sender }) => sender);
		...
		return Event.map(onHello, webContents => {
			...
			const onMessage = createScopedOnMessageEvent(id, `${projectName}:message`) as Event<VSBuffer>;
			const onDidClientDisconnect = Event.any(Event.signal(createScopedOnMessageEvent(id, `${projectName}:disconnect`)), onDidClientReconnect.event);
